# Current dashboard style audit (branch `openclaw/ui-modern-dashboard-rebuild`)

## Snapshot source

- File (from branch): `src/components/screens/ReformistDashboardScreen.tsx`

## What was modernized

- Dashboard container applies modern DS surface + content tokens (`--ds-bg-base`, `--ds-content-*`) via Tailwind arbitrary values.
- Cards are composed using `Card` variants (`surface`/`raised`) and semantic `Badge status=...` (including `dot` in hero).
- KPI section avoids the legacy `MetricCard` accent-bar visual language by composing KPI cards directly with `Card` + overline + big value.

## What still looks inherited / not fully backed by the spec

- `Card` geometry still uses the app adapter defaults (`rounded-2xl`) rather than the modern `radius/lg` (~16px).
- Typography is not wired to Inter + Space Grotesk at the layout/font level, so “display/num” intent is not guaranteed.
- Some borders still use legacy border color tokens (because the app expects `--border-*` as colors).

## Token mixing (observed)

**DS tokens referenced:**
...
**Non-DS tokens referenced (legacy or other):**
...
## Components likely to replace/redesign next (once spec is final)

- `MetricCard`: decide whether to adopt the modern ZIP `MetricCard` and drop the accent bar variant.
- `ProgressBar`: ensure warning/primary semantics follow the “only amber when real warning” rule.
- `ListItem`: confirm it matches the modern ListItem/ListGroup patterns (separators, densities).

## Design decisions currently not clearly backed by `SISTEMA-DE-DISENO.md`

- Whether KPI cards should show semantic badges like “info/warning/danger” as raw text, or use a localized label (e.g., “EN CURSO”, “AVISO”).
- Whether the dashboard should have a distinct header treatment (like the phone mockups) or remain web-first.

## Dashboard code (for reviewer reference)
