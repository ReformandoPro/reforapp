...
> **Update note (Openclaw integration filter):** `/design-reference` already exists in `main` as an internal, safe route (see `merge: add internal design reference route`). This spec must remain **dashboard-local**: any atmospheric container/radial layers must be implemented only inside `src/components/screens/ReformistDashboardScreen.tsx` (or dashboard-local CSS module if explicitly approved), and must **not** touch `globals.css`, `AppShell`, `layout.tsx`, `body` or other global surfaces.
