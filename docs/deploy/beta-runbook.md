...
1. **Backup previo:** ejecutar `Supabase Staging Ops (manual)` con `mode=backup_only`; descargar el artefacto y registrar su nombre, hash, ubicación y timestamp.
2. **Aplicar migraciones:** confirmar el project ref y ejecutar `mode=db_push` con `backup_confirmed=I_HAVE_DOWNLOADED_BACKUP`. Conservar la salida literal y confirmar que el destino es el estado real de beta.
...
4. **Desplegar aplicación:** ejecutar `Deploy app-beta` sobre el SHA aprobado. El destino debe ser únicamente `reformando-app-beta` en `/docker/reformando-app-beta`.
5. **Health check:** consultar `/api/health` con Basic Auth autorizada. Exigir HTTP 200, JSON válido, `status: "ok"` y el SHA esperado.
...
- Si falla la migración: detenerse, conservar logs y no desplegar la aplicación.
