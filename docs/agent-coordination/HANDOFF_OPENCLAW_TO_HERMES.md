...
Hermes puede usar `ProjectCard` para tarjetas/listados de obra, pero no debe asumir todavía persistencia real en Supabase ni campos adicionales que no estén expuestos por el contrato. `ProjectCard` sigue siendo el contrato estable de UI. El primer candidato previsto para salir de mock será `getProjectCards()`; su primera lectura real se ha decidido como **parcial**, alimentada inicialmente solo desde `projects` y `clients`. Ya existe un esqueleto futuro de `SupabaseProjectsRepository`, un mapper interno de repository para esa transformación parcial, un helper interno para mapear listas de filas, un borrador documental de migración mínima futura, un query plan documental, una decisión arquitectónica sobre organization scope, un stub no conectado del flujo futuro, una decisión sobre application context, un diseño documental del futuro application/repository gateway, una **factory** en `src/lib/application/repositories/projects-repository-factory.ts`, una **factory** en `src/lib/application/repositories/dashboard-repository-factory.ts`, una **factory** en `src/lib/application/repositories/budgets-repository-factory.ts` y un **shape mínimo de context** en `src/lib/application/context/projects-application-context.ts`. Las factories aceptan un `dataSource` conceptual, pero siguen teniendo solo `mock` como ruta operativa real. Si se pide `supabase` desde cualquiera de ellas, fallan de forma explícita y controlada; no devuelven mock silenciosamente ni implican que Supabase ya esté activo. `services/projects.ts`, `services/dashboard.ts` y `services/budgets.ts` ya consumen estas factories, pero lo hacen forzando `dataSource: "mock"`, así que el comportamiento visible no cambia. Hermes no debe asumir que la query ya exista, que las tablas ya existan o depender del shape intermedio de Supabase. Hermes no debe importar repositories concretos, no debe depender de mock/Supabase y no debe introducir `organizationId` en UI. Debe seguir consumiendo `services`. El mock sigue siendo la implementación activa. Antes de conectar Supabase real, Openclaw prepara ese read parcial dentro de `service -> application -> repository`, no en UI, y el scope de organización seguirá siendo responsabilidad futura de la capa data/application. La UI no debe acoplarse al SQL conceptual. Importante: este cambio no toca el motor presupuestario ni su lógica de cálculo; solo cambia la composición service/repository.
