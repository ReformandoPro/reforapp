...
Hermes puede usar `ProjectCard` para tarjetas/listados de obra, pero no debe asumir todavía persistencia real en Supabase ni campos adicionales que no estén expuestos por el contrato. `ProjectCard` sigue siendo el contrato estable de UI. El primer candidato previsto para salir de mock será `getProjectCards()`; su primera lectura real se ha decidido como **parcial**, alimentada inicialmente solo desde `projects` y `clients`. Ya existe un esqueleto futuro de `SupabaseProjectsRepository`, un mapper interno de repository para esa transformación parcial, un helper interno para mapear listas de filas, un borrador documental de migración mínima futura, un query plan documental, una decisión arquitectónica sobre organization scope, un stub no conectado del flujo futuro, una decisión sobre application context, un diseño documental del futuro application/repository gateway y ahora también una **factory mínima no conectada** en `src/lib/application/repositories/projects-repository-factory.ts`. Esa factory devuelve hoy el mock, prepara el futuro punto de composición y no cambia runtime. Hermes no debe asumir que la query ya exista, que las tablas ya existan o depender del shape intermedio de Supabase. Hermes no debe importar repositories concretos, no debe depender de mock/Supabase y no debe introducir `organizationId` en UI. Debe seguir consumiendo `services`. `services/projects.ts` sigue sin usar la nueva factory; el mock sigue siendo la implementación activa. Antes de conectar Supabase real, Openclaw prepara ese read parcial dentro de `service -> repository`, no en UI, y el scope de organización seguirá siendo responsabilidad futura de la capa data/application. La UI no debe acoplarse al SQL conceptual.
