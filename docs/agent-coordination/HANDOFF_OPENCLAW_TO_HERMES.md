...
Hermes puede usar `ProjectCard` para tarjetas/listados de obra, pero no debe asumir todavía persistencia real en Supabase ni campos adicionales que no estén expuestos por el contrato. El primer candidato previsto para salir de mock será `getProjectCards()`; su primera lectura real se ha decidido como **parcial**, alimentada inicialmente solo desde `projects` y `clients`. Hasta que Openclaw implemente ese repository real, y en todo lo relativo a contadores, sigue siendo dato mock encapsulado.
